const executive1 = a => {
	...
	(a => {
		...
		const mapColor = {excellence:'#999A9E', Good:'#D3AF3D', Fair:'#D9D9D9', 'Poor/Bad':'#B28E65'}
		
		m.ekse.map.attributionControl.setPrefix('')
		fetch('map1.json').then(a=>a.json()).then(a=>{
			//buang loading
			// (a => { a.parentElement.removeChild(a) })(document.getElementById('loadingEkseMap'))
		...
		el({a:'span', b:el({a:'div', b:a, d:{id:'loadingEkseMap', style:'position:absolute; margin-top:-39.5vh; margin-left:0vw; height:34vh; width:44vw; background:rgba(0,0,0,0.5); border-radius:9px; display:flex;align-items:center;justify-content:center;'}}), d:{class:'loader'} })
	})(a);
	...
	(a => {
		...
		const b = el({a:'canvas', b:el({a:'div', b:a, d:{style:'position:relative; margin:0; padding:0 0.5vw; width:24vw; height:36.5vh; display:flex; align-items:center; justify-content:center;'}})})
	...
	(a => {
		...
		})(el({a:'div', b:a, d:{style:'position:relative; margin:0; padding:0.5vh 1vw 0 2vw; width:29vw; height:25vh; display:flex; align-items:center; justify-content:center;'}}));
	...
	(a => {
		...
		el({a:'span', b:el({a:'div', b:a, d:{id:'loadingEkseExp', style:'position:absolute; margin-top:0vh; margin-left:-1vw; height:38vh; width:44vw; background:rgba(0,0,0,0.5); border-radius:9px; display:flex;align-items:center;justify-content:center;'}}), d:{class:'loader'} })

		fetch('api.php?cmd=cx-experience').then(a=>a.json()).then(b => {
			(a => { a.parentElement.removeChild(a) })(document.getElementById('loadingEkseExp'))
			if (b.data.pl) {
	...
	(a => {
		...
		el({a:'span', b:el({a:'div', b:a, d:{id:'loadingEkseCdn', style:'position:absolute; margin-top:-3vh; margin-left:0vw; height:39vh; width:24vw; background:rgba(0,0,0,0.5); border-radius:9px; display:flex;align-items:center;justify-content:center;'}}), d:{class:'loader'} })
		...
		b.width = parseInt(b.parentElement.getBoundingClientRect().width)
		b.height = parseInt(b.parentElement.getBoundingClientRect().height)
		const ilogo = ['img/yt.svg', 'img/fb.svg', 'img/netflix.svg', 'img/tt.svg'].map(a => {
			...
			plugins: [{
				afterDraw: a => {
					...
					yAxis.ticks.forEach((value, index) => {
						ctx.drawImage(ilogo[index], xAxis.left - (ilogo[index].width + 5 + (index==2?180:0)), yAxis.getPixelForTick(index) - 9)
					})
				}
			}],
			data: {
				labels: ['Youtube', 'facebook', 'netflix', 'Titok'],
				datasets: [{
					label: 'Global',
	...
	(a => {
		...
		(a => {
			...
			el({a:'span', b:el({a:'div', b:a, d:{id:'loadingEkseCity', style:'position:absolute; margin-top:13vh; margin-left:0vw; height:41vh; width:28vw; background:rgba(0,0,0,0.5); border-radius:9px; display:flex;align-items:center;justify-content:center;'}}), d:{class:'loader'} })
	...
	fetch("data/sla-performance.json").then((a) => a.json()).then((b) => {
		...
		m.ekse.card3Data.tr = {
			'4g': {
				tpl: b.data['4g'].pl.slice(-1)[0]['target_sla'],
				rpl: b.data['4g'].pl.slice(-1)[0]['realisasi'],
				tlat: b.data['4g'].lat.slice(-1)[0]['target_sla'],
				rlat: b.data['4g'].lat.slice(-1)[0]['realisasi'],
				tjitt: b.data['4g'].jitt.slice(-1)[0]['target_sla'],
				rjitt: b.data['4g'].jitt.slice(-1)[0]['realisasi'],
			},
			cti: {
				tpl: b.data['cti'].pl.slice(-1)[0]['target_sla'],
				rpl: b.data['cti'].pl.slice(-1)[0]['realisasi'],
				tlat: b.data['cti'].lat.slice(-1)[0]['target_sla'],
				rlat: b.data['cti'].lat.slice(-1)[0]['realisasi'],
				tjitt: b.data['cti'].jitt.slice(-1)[0]['target_sla'],
				rjitt: b.data['cti'].jitt.slice(-1)[0]['realisasi'],
			}
		}
		
		m.ekse.targetPL4g.parentElement.style.background = parseFloat(m.ekse.card3Data.tr['4g'].tpl) > parseFloat(m.ekse.card3Data.tr['4g'].rpl) ? 'red' : 'green'
		...
		m.ekse.targetLat4g.parentElement.style.background = parseFloat(m.ekse.card3Data.tr['4g'].tlat) > parseFloat(m.ekse.card3Data.tr['4g'].rlat) ? 'red' : 'green'
		...
		m.ekse.targetJitt4g.parentElement.style.background = parseFloat(m.ekse.card3Data.tr['4g'].tjitt) > parseFloat(m.ekse.card3Data.tr['4g'].rjitt) ? 'red' : 'green'
		...
		m.ekse.targetPLCTI.parentElement.style.background = parseFloat(m.ekse.card3Data.tr['cti'].tpl) > parseFloat(m.ekse.card3Data.tr['cti'].rpl) ? 'red' : 'green'
		...
		m.ekse.targetLatCTI.parentElement.style.background = parseFloat(m.ekse.card3Data.tr['cti'].tlat) > parseFloat(m.ekse.card3Data.tr['cti'].rlat) ? 'red' : 'green'
		...
		m.ekse.targetJittCTI.parentElement.style.background = parseFloat(m.ekse.card3Data.tr['cti'].tjitt) > parseFloat(m.ekse.card3Data.tr['cti'].rjitt) ? 'red' : 'green'
		...
		(a => { a.parentElement.removeChild(a) })(document.getElementById('loadingEkseCard3'))
	})
