const sla2tabel3 = a => {
	...
	(a => {
		el({a:'div', b:a, c:'SLA LATENCY 4G W04'})
		...
		const addHeader = a => {
			(a => {
				...
				el({a:'th', b:a, c:'TARGET SLA LATENCY', d:{rowspan:2} })
		...
		fetch("api.php?cmd=weekly-sla-latency").then((a) => a.json()).then((b) => {
			...
			a.previousElementSibling.textContent = `SLA PACKET LOSS 4G BBC W${week_num}`
